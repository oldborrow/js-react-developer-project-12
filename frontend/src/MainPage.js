...
function MainPage() {
  ...
  const [modifiableChannelId, setModifiableChannelId] = useState(null);
  const deleteChannel = () => {
  ...
  return (
    <Container>
      ...
      <Row xs={2} md={4} lg={6}>
          ...
          <ListGroup>
            ...
            {messengerInfo.channels.map((ch) => (ch.id === 1 || ch.id === 2 ? <ListGroup.Item key={ch.id}><Button onClick={() => changeChannel(ch.id)}>{ch.name}</Button></ListGroup.Item> : (
              <ListGroup.Item key={ch.id}>
                ...
                {' '}
                <Button onClick={() => onOpenModifyChannel(ch.id)}>Управление каналом</Button>
              </ListGroup.Item>
            )))}
        ...
        <Col sm={8}>
          {messengerInfo.messages.map((m) => (m.channelId === messengerInfo.channelId ? (
            <div key={m.id}>
              {m.username}
              :
              {' '}
